...
export const createPost = async (req, res) => {
  try {
    ...
    // get tag ids based on category name
    let idst = [];
...
export const editPost = async (req, res) => {
  try {
    ...
    const { title, content, featuredImage, categories, tags } = req.body;
    // get category ids based on category name
    let ids = [];
    ...
    for (let i = 0; i < categories.length; i++) {
     ...
     // get tag idst based on tag name
     let idst = [];
...
export const postsForAll = async (req, res) => {
  try {
    const posts = await Post.find().select("title slug content")
    .populate("featuredImage")
    .sort({ createdAt: -1 });
    res.json(posts);
  } catch (err) {
...
export const getNumbers = async (req, res) => {
  try {
    ...
    return res.json({ posts, users, comments, categories, tags });
  } catch (err) {
    console.log(err);
  }
};
